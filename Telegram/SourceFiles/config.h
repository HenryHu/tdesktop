...
constexpr str_const AppId =
    "{53F49750-6209-4FBF-9CA8-7A333C87D1ED}";  // used in updater.cpp and
                                               // Setup.iss for Windows
constexpr str_const AppFile = "Telegram";

enum {
    MTPIdsBufferSize =
        400,  // received msgIds and wereAcked msgIds count stored
    MTPCheckResendTimeout = 10000,  // how much time passed from send till we
                                    // resend request or check it's state, in ms
    MTPCheckResendWaiting =
        1000,  // how much time to wait for some more requests, when resending
               // request or checking it's state, in ms
    MTPAckSendWaiting = 10000,  // how much time to wait for some more requests,
                                // when sending msg acks
    MTPResendThreshold = 1,   // how much ints should message contain for us not
                              // to resend, but to check it's state
    MTPContainerLives = 600,  // container lives 10 minutes in haveSent map

    MTPKillFileSessionTimeout =
        5000,  // how much time without upload / download causes additional
               // session kill
    ...
    MaxPhoneTailLength =
        32,  // rest of the phone number, without country code (seen 12 at
             // least), need more for service numbers
    ...
    SetOnlineAfterActivity = 30,  // user with hidden last seen stays online for
                                  // such amount of seconds in the interface
...
inline const GUID &cGUID() {
    ...
    static const GUID gGuid = {
        ...
        0xe370,
        0x4cde,
        {0x98, 0xd3, 0xac, 0xc1, 0x10, 0xc5, 0x96, 0x7d}};
#else   // OS_MAC_STORE
    static const GUID gGuid = {0xe51fb841,
                               0x8c0b,
                               0x4ef9,
                               {0x9e, 0x9e, 0x5a, 0x0, 0x78, 0x56, 0x76, 0x27}};
#endif  // OS_MAC_STORE
...
static const BuiltInDc _builtInTestDcs[] = {{1, "149.154.175.10", 443},
...
inline int builtInDcsCount() {
    return (cTestMode() ? sizeof(_builtInTestDcs) : sizeof(_builtInDcs)) /
           sizeof(BuiltInDc);
}
...
inline int builtInDcsCountIPv6() {
    return (cTestMode() ? sizeof(_builtInTestDcsIPv6)
                        : sizeof(_builtInDcsIPv6)) /
           sizeof(BuiltInDc);
}
...
enum {
    ...
    UseBigFilesFrom =
        10 * 1024 *
        1024,  // mtp big files methods used for files greater than 10mb
    ...
    DocumentUploadPartSize1 =
        64 * 1024,  // 64kb for little document ( <= 32mb )
    DocumentUploadPartSize2 =
        128 * 1024,  // 128kb for small document ( <= 375mb )
    DocumentUploadPartSize3 =
        256 * 1024,  // 256kb for medium document ( <= 750mb )
    DocumentUploadPartSize4 =
        512 * 1024,  // 512kb for large document ( <= 1500mb )
    UploadRequestInterval =
        500,  // one part each half second, if not uploaded faster

    MaxPhotosInMemory =
        50,  // try to clear some memory after 50 photos are created
    NoUpdatesTimeout = 60 * 1000,  // if nothing is received in 1 min we ping
    NoUpdatesAfterSleepTimeout =
        60 *
        1000,  // if nothing is received in 1 min when was a sleepmode we ping
    WaitForSkippedTimeout = 1000,  // 1s wait for skipped seq or pts in updates
    WaitForChannelGetDifference = 1000,  // 1s wait after show channel history
                                         // before sending getChannelDifference

    MemoryForImageCache =
        64 * 1024 *
        1024,  // after 64mb of unpacked images we try to clear some memory
    IdleMsecs =
        60 * 1000,  // after 60secs without user input we think we are idle

    SendViewsTimeout = 1000,  // send views each second

    ForwardOnAdd = 100,  // how many messages from chat history server should
                         // forward to user, that was added to this chat
};
